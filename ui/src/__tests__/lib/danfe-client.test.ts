...
const client = apiClient as unknown as {
  get: (path: string, config?: {timeout: number}) => Promise<AuxiliaryDocumentDownload>
}
...
describe('PDF download client methods', () => {
  ...
  it('downloadNfceDanfe returns the cached S3 download descriptor', async () => {
    const download = {url: 'https://s3.example/nfce.pdf', expires_at: '2026-08-29T12:00:00Z', cached: true}
  ...
  it('downloadMdfeDamdfe returns the cached S3 download descriptor', async () => {
    const download = {url: 'https://s3.example/mdfe.pdf', expires_at: '2026-08-29T12:00:00Z', cached: false}
    const spy = vi.spyOn(client, 'get').mockResolvedValue(download)
  ...
  it('downloadNfeDanfe returns the cached S3 download descriptor', async () => {
    const download = {url: 'https://s3.example/nfe.pdf', expires_at: '2026-08-29T12:00:00Z', cached: true}
    const spy = vi.spyOn(client, 'get').mockResolvedValue(download)
